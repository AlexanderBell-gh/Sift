      ...
      {isTrial && (
        <div className="bg-gradient-to-r from-amber-500 to-orange-500 text-white text-center py-2 px-4 text-sm font-medium relative z-50">
          {isTrialExpired ? (
            ...
            <>Free trial • {trialHoursRemaining} hour{trialHoursRemaining !== 1 ? 's' : ''} remaining • <button onClick={() => { signOut(); navigate('/', { state: { tab: 'signup' } }); }} className="underline hover:no-underline font-semibold">Sign up now</button></>
          )}
        </div>
      )}
